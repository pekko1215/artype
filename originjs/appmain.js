...
function main() {
    ...
    slotmodule.on("allreelstop", function(e) {
        ...
        if(gamemode == 'normal' && bonusflag == 'none'){
            switch(rt.mode){
                case 'リーチ目高確率':
    ...
    function effect(lot,orig) {
        ...
        switch(rt.mode){
                ...
                if(orig == 'ベル'){
                    ...
                    function timer(){
                        setTimeout(()=>{
                            if(stoped)return;
                            $('.nabi').removeClass('on');
                            $('#nabi'+(idx + 1)).addClass('on');
                            idx = (idx + 1)%3;
                            timer();
                        },150)
                    }
                    timer();
                    slotmodule.once('reelstop',e=>{
                        stoped = true;
                        $('.nabi').removeClass('on');
                        if(e.reel === 1){
                            if(slotmodule.playControlData.reelSliped[1] === 0){
                                sounder.playSound('bita');
                                atEffect(lot);
                                atEffect('3択子役'+(rand(3)+1));
                            }
                        }
                    })
                }
            ...
            case 'BAR揃え待機':
                ...
                break
            case 'リプレイ高確率':
                segments.effectseg.setSegments(rt.game);
                if(orig == 'ベル'){
                    atEffect(lot);
                    sounder.playSound('yokoku')
                }
